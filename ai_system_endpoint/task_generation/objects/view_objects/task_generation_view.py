    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            response = requests.post(
                ...
                + "/task_generation_model/case_tmp_storage/",
...
class BackupView(APIView):
    ...
    def post(self, request):
        try:
            response = requests.post(
                url=settings.AI_BACKEND_URL + "/task_generation_model/backup/",
                headers={"Authorization": f"Bearer {settings.AI_BACKEND_API_KEY}"},
            )
            return Response(response.json(), status=status.HTTP_200_OK)
        except requests.exceptions.ConnectionError:
    ...
    def post(self, request):
        try:
            ...
            if restore_name is None:
                ...
                response = requests.post(
                    url=settings.AI_BACKEND_URL + "/task_generation_model/rollback/",
                    headers={"Authorization": f"Bearer {settings.AI_BACKEND_API_KEY}"},
                    data={"hash": restore_name},
                )
                return Response(response.json(), status=status.HTTP_200_OK)
    ...
    def get(self, request):
        try:
            ...
            response = requests.get(
                url=settings.AI_BACKEND_URL
                + f"/task_generation_model/history/?page={page_number}",
                headers={"Authorization": f"Bearer {settings.AI_BACKEND_API_KEY}"},
            )
            return Response(response.json(), status=status.HTTP_200_OK)
        except requests.exceptions.ConnectionError:
        ...
        try:
            ...
            for job in job_dict["jobs"]:
                if job["status"] == "queued" or job["status"] == "running":
                    return Response(
                        {
                            "error": "Only one training job is allowed to run a single time"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            ...
            gradient_accumulation_steps = request.POST.get(
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            job_scheduler.add_job(
                ...
                name="Model_Reset",
                url=settings.AI_BACKEND_URL
                + "/task_generation_model/restore_baseline/",
                headers={"Authorization": f"Bearer {settings.AI_BACKEND_API_KEY}"},
                data=form_data,
                timeout=None,
            )
...
class CurrentBackupVersionView(APIView):
    ...
    def get(self, request):
        try:
            response = requests.get(
                url=settings.AI_BACKEND_URL
                + f"/task_generation_model/current_backup_version/",
                headers={"Authorization": f"Bearer {settings.AI_BACKEND_API_KEY}"},
            )
            return Response(response.json(), status=status.HTTP_200_OK)
        except requests.exceptions.ConnectionError:
...
class CurrentModelIdView(APIView):
    ...
    def get(self, request):
        try:
            response = requests.get(
                url=settings.AI_BACKEND_URL
                + f"/task_generation_model/current_model_id/",
                headers={"Authorization": f"Bearer {settings.AI_BACKEND_API_KEY}"},
            )
            return Response(response.json(), status=status.HTTP_200_OK)
        except requests.exceptions.ConnectionError:
