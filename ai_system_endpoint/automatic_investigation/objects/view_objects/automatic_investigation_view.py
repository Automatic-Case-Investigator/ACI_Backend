...
class AutomaticInvestigationView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        max_queries_per_iteration = int(data.get("max_queries_per_iteration"))
        ...
        if max_queries_per_iteration > 20:
            max_queries_per_iteration = 20
        elif max_queries_per_iteration <= 0:
...
class SingleActivityInvestigationView(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not data.get("max_queries_per_iteration").isdigit():
            return Response(
                {"error": 'Parameter "max_queries_per_iteration" is not an integer'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # Add to job queue for single activity investigation
        job_scheduler.add_job(
            investigator.investigate_single_activity,
            name="Single_Activity_Investigation",
