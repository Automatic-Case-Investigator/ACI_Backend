...
class ObservableControlView(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, *args, **kwargs):
        soar_id = request.GET.get("soar_id")
        org_id = request.GET.get("org_id")
        observable_id = request.GET.get("observable_id")
        case_id = request.GET.get("case_id")

        if soar_id is None or (observable_id is None and case_id is None):
            return Response(
                {"error": "Required field missing"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if observable_id is not None:
            task_data = soar_wrapper.get_observable(org_id, observable_id)
            if "error" in task_data.keys():
                return Response(task_data, status=status.HTTP_502_BAD_GATEWAY)

            return Response(task_data, status=status.HTTP_200_OK)
        else:
            tasks_data = soar_wrapper.get_observables(org_id, case_id)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        observable_id = request.data.get("observable_id")

        if soar_id is None or observable_id is None:
            return Response(
                {"error": "Required field missing"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        response = soar_wrapper.delete_observable(observable_id)
